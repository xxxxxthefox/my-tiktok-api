...
from flask_cors import CORS  # <-- إضافة هذا السطر
import requests
from user_agent import generate_user_agent
import os

app = Flask(__name__)
CORS(app)  # <-- إضافة هذا السطر للسماح بجميع الاتصالات من أي مكان
...
def send_likes():
    ...
    try:
        ...
        response = requests.post(
            'https://leofame.com/ar/free-tiktok-likes?api=1', 
            cookies=cookies, 
            headers=headers, 
            data=data,
            timeout=20
        ...
        if "success" in response.text:
            return jsonify({"status": "success", "message": f"DONE : {url}"})
        else:
            return jsonify({"status": "failed", "message": f"حاول بعد 24 ساعه : {url}"})
